...
#Quick Hack: used to convert e.g. "C:\Program Files\Toolbox2docker" to "/c/Program Files/Toolbox2docker"
win_to_unix_path(){ 
	wd="$(pwd)"
	cd "$1"
		the_path="$(pwd)"
...
# by Toolbox2docker over-ride binaries provided by
...
if [ ! -f "${DOCKER_MACHINE}" ]; then
  echo "Docker Machine is not installed. Please re-run the Toolbox2docker Installer and try again."
  exit 1
fi

if [ ! -f "${VBOXMANAGE}" ]; then
  echo "VirtualBox is not installed. Please re-run the Toolbox2docker Installer and try again."
  exit 1
fi
